...
- Name: mouse.spec.ts >> Mouse Hover trial
...
Locator: getByText('You hovered over the Button')
Expected: visible
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" getByText('You hovered over the Button') with timeout 5000ms
  - waiting for getByText('You hovered over the Button')
...
```ts
  ...
  5  | 
  6  |     await page.locator('#toolTipButton').hover({ force: true });
  7  |     await page.waitForTimeout(1000);
  8  | 
  9  |    // await page.locator('#toolTipButton').hover();
  10 |     console.log(await page.content());
> 11 |     await expect(page.getByText('You hovered over the Button')).toBeVisible();
     |                                                                 ^ Error: expect(locator).toBeVisible() failed
  12 | });
  13 | 
  14 | 
  15 | test('Right Click', async ({ page }) => {
  16 |     await page.goto('https://demoqa.com/buttons');
  17 |     await page.locator('#rightClickBtn').click({button: 'right'});
  18 |     await expect(page.locator('#rightClickMessage')).toContainText('You have done a right click');
  19 | });
  20 | 
  21 | test('Double Click', async ({ page }) => {
  22 |     await page.goto('https://demoqa.com/buttons');
  23 |     await page.locator('#doubleClickBtn').dblclick();
  24 |     await expect(page.locator('#doubleClickMessage')).toContainText('You have done a double click');
  25 | });
  26 | 
```