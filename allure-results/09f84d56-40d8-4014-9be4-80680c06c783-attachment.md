...
- Location: tests\radio.spec.ts:11:5
...
```ts
  ...
  13 |     // Expand all
> 14 |     await page.locator('button[title="Expand all"]').click();
     |                                                      ^ Error: locator.click: Test timeout of 30000ms exceeded.
  15 |     await page.pause()
  16 |     // Select Downloads checkbox
  17 |     await page.locator('//span[text()="Downloads"]').click();
  18 |     await page.pause()
  19 |     await expect(page.locator('#result')).toContainText('downloads');
  20 | });
```