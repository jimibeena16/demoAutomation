...
- Location: tests\demo2.spec.ts:48:5
...
> 50  |   await page.locator('#user-name').fill('standard_user');
  ...
  106 |   await expect(sportsCheckbox).not.toBeChecked();
  107 |   await expect(readingCheckbox).not.toBeChecked();
  108 | 
  109 |   // Select a checkbox by clicking its corresponding <label>
  110 |   await page.locator('label[for="hobbies-checkbox-1"]').click();
  111 | 
  112 |   // 2. Verify 'toBeChecked()' becomes true for Sports, but remains false for Reading
  113 |   await expect(sportsCheckbox).toBeChecked();
  114 |   await expect(readingCheckbox).not.toBeChecked();
  115 | 
  116 |   // Check multiple boxes
  117 |   await page.locator('label[for="hobbies-checkbox-2"]').click();
  118 |   await expect(readingCheckbox).toBeChecked();
  119 | });
  120 | 
  121 | 
  122 | 
  123 | test('Generic Value Assertions Example', async ({ page }) => {
  124 |   await page.locator('#user-name').fill('standard_user');
  125 |   await page.locator('#password').fill('secret_sauce');
  126 |   await page.locator('#login-button').click();
  127 | 
  128 |   // Extract total items programmatically using evaluate/count
  129 |   const itemCount = await page.locator('.inventory_item').count();
  130 | 
  131 |   // Generic assertion on raw number
  132 |   expect(itemCount).toBe(6);
  133 |   expect(itemCount).toBeGreaterThan(0);
  134 |   expect(itemCount).toBeLessThan(10);
  135 | 
  136 |   // Extract text content into a string variable and test it
  137 |   const badgeText = await page.locator('.shopping_cart_badge').isVisible().catch(() => false);
  138 |   expect(badgeText).toBeFalsy(); // Cart badge shouldn't exist initially
  139 | });
  140 | 
  141 | 
  142 | 
